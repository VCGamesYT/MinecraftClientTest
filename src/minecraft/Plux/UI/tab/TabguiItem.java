...
public final class TabguiItem {
    ...
    public void drawTabMenu(Minecraft mc2, int x2, int y2) {
        ...
        while (i2 < this.Mods.size()) {
            ...
            Minecraft.getMinecraft().fontRendererObj.drawString(String.valueOf(String.valueOf(/*this.Mods.get(i2).getMod().isToggled() ? this.gui.getColourHightlight() :*/ this.gui.getColourNormal())) + this.Mods.get(i2).getMod().getName(), x2 + 2, y2 + this.gui.getTabHeight() * i2 - 1 + 2, -1);
            boxY += 12;
            ++i2;
        }
    }
    ...
    private void countMenuSize(Minecraft mc2) {
        ...
        for (GuiItem Mod : this.Mods) {
            if (Minecraft.getMinecraft().fontRendererObj.getStringWidth(Mod.getMod().getName()) <= maxWidth) continue;
            maxWidth = Minecraft.getMinecraft().fontRendererObj.getStringWidth(Mod.getMod().getName()) + 4;
        }
